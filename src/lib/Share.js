...
export const shareStatus = (puzzle, score, completedPuzzles, completionTimes) => {
    let shareText = (
        '24 game - '+ getMonthAndDay() +'\n' +
        'Puzzles solved: ' + score + '😤\n\n'
    )
    
    for (let i = 0; (i < score && i < 4); i++) {
        shareText += completedPuzzles[i].join(' ') + " in " + (completionTimes[i] / 1000).toFixed(2) + "s\n";
    }

    if (score > 4){
       shareText += '...\n' 
       shareText += completedPuzzles[score - 1].join(' ') + " in " + (completionTimes[score-1] / 1000).toFixed(2) + "s\n";
    }
